  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-gray-800/50 rounded-lg p-3 text-xs space-y-1.5">
        <p className="text-gray-500 uppercase tracking-widest text-xs mb-2">場所の凡例</p>
        {[
          ["#14532d", "E", "入口 (Entry)"],
          ["#854d0e", "T", "食券機 (Ticket)"],
          ["#7c2d12", "C", "カウンター (Counter)"],
          ["#1e3a5f", "",  "座席 (Seat)"],
          ["#581c87", "R", "返却台 (Return)"],
        ].map(([color, letter, label]) => (
          <div key={label} className="flex items-center gap-2">
            <span className="w-3 h-3 rounded flex-shrink-0 text-center leading-3 text-[8px] font-bold" style={{ background: color, color: "#fff" }}>
              {letter}
            </span>
      ...
      {/* Legend – Agent states */}
      <div className="bg-gray-800/50 rounded-lg p-3 text-xs space-y-1.5">
        <p className="text-gray-500 uppercase tracking-widest text-xs mb-2">状態の凡例</p>
        {[
          ["#a3e635", "入場中"],
          ["#facc15", "食券購入中"],
          ["#38bdf8", "席へ移動中"],
