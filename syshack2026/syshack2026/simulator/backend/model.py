...
class CafeteriaModel(Model):
    ...
    def join_counter_queue(self, student: StudentAgent) -> None:
        """Student orders from their seat — joins shortest counter queue."""
        ...
        for i in range(self.n_ticket_machines):
            if self._ticket_timers[i] > 0:
                ...
                if student.state == AgentState.BUYING_TICKET:
                    # Ticket issued → claim seat and go sit down
                    seat = self.claim_seat()
                    if seat:
                        student.seat_pos = seat
                        student.state = AgentState.FINDING_SEAT
                        student.path = self.find_path(student.pos, seat)
                    else:
                        # No seats available → abandon
                        student.state = AgentState.EXITING
                        exit_pos = random.choice(self.layout.entry_positions)
                        student.path = self.find_path(student.pos, exit_pos)
